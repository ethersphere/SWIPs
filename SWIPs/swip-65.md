...
(forthcoming)") — but freestanding: nothing in the pattern requires BPS. SWIP-60 enters
only through the carriage rule in the BPS section below. -->

- **Business line**: any append-only stream on Swarm becomes **self-indexing** — every
  update carries the complete index of its history, so late joiners, seekers and
  after-the-fact readers need only the latest update, and a missed update is
  recoverable from storage, not lost. Headline applications: HLS-style live video with
  no manifest republication (over BPS, where every live message doubles as a feed
  update), and collaborative editing with catch-up.
- **Dev line**: no new wire frames and no proto change; implement (1) publisher-side
  whirl-only pot maintenance (already exists in bee-js), (2) follower-side gap
  detection and recovery — for any feed client, BPS or not — (3) the `FEED_TOPIC`
  bare-index carriage rule per SWIP-60's API
  ([#104](https://github.com/ethersphere/SWIPs/pull/104)), groundwork in bee
  [#5435](https://github.com/ethersphere/bee/pull/5435); done per the conformance
  section.
- **DISC: NO** — SOCs, feeds, postage, push-sync and retrieval are all used exactly as
  they are; this is a chunk-payload convention plus client behaviour. No storage-layer
  change, no new protocol; the BPS integration adds no frames.

## Simple Summary

A **self-indexed feed** is an ordinary sequential feed whose update payloads are nodes
of a **whirl-only pot** over all previous updates: the feed carries its own index. The
latest update alone gives ordered playback, random access and range queries over the
entire history, with zero auxiliary chunks — and recovery of missed updates rides the
same structure: the update that reveals a gap carries the pot that indexes everything
missed, and the descent authenticates the recovered chunks by hash against the one
signature already verified. The pattern is transport-agnostic: any feed client can
publish and follow one. Carried over BPS (SWIP-60), it does double duty — each live
message is simultaneously a retrievable feed update, transmitted as the bare index
(the topic being implicit in the channel), so an index gap is visible at the very next
frame.

## Motivation

Every streaming design on Swarm trips over the same question: how does a late joiner
get the history without the publisher republishing an ever-growing manifest? The
whirl-only pot answers it at the theoretical floor — one chunk per update, and that
chunk is the update. The construction was instigated by the BPS decomposition
(SWIP-60 deliberately left the sequential construction open: under explicit publishers
the id is unconstrained, and publishers MAY use sequence numbers), and it settles three
BPS questions at once — dedup needs no per-binding special cases, missed live messages
stop being unrecoverable, and SWIP-61's dual-parent bandwidth cost becomes a choice —
but nothing in it requires BPS.
...
Byte-for-byte an ordinary sequential feed update — any feed client publishes and
follows it, and update `j` is individually addressable at `H( H(TOPIC ‖ j) ‖ OWNER )`.
Everything this SWIP adds sits in the `PAYLOAD` and in follower behaviour; no
transport, broker or cohort is presupposed.
...
The pot layer is a **profile**: a stream whose payloads need no history (pure signal)
MAY run the bare sequential feed alone — it keeps gap detection, and recovers by
...
- **the gap-revealing update is the recovery index** — no id reconstruction, no feed
...
- **implicit authentication** — fork references are content addresses, so every
  ...
  signature was already checked. One signature covers the whole recovery; the missed
  updates need no individual signature verification, because they are implicitly
  signed via the pot node that references them.
...
traversal assigns indices by **counting from a known anchor** — the index of the
update whose payload roots the descent, known to any follower because it constructed
the lookup (or, over BPS, read it off the frame).

Recovered elements MUST be indistinguishable to the application from ones delivered in
sequence (same session, same serialization, delivered in index order). Recovery is
silent self-healing, not an error path.
...
feed update — one retrieval *and one signature check* per missed update.

### Equivocation and the anchor check

Two valid SOCs sharing an index but not a payload are an **equivocation**: distinct
chunk addresses, and the pair is self-evidencing publisher misbehaviour — two
signatures by one owner over one id. What to do about it is the application's
decision; detecting and signalling it is the protocol's. Detection is an **anchor
check**: the follower validates that the SOC at its last known index wraps the very
pot node it holds from the chain — one comparison that implicitly validates the
integrity of the entire pot since the follower joined. On mismatch the follower MUST
surface a protocol error to the application. The anchor check is not a history audit:
recovery by pot descent does not retrieve the missed SOCs, so a fork at those indices
— a divergent signed history — surfaces only if the application retrieves and compares
them; how much of that to pay is its integrity/latency trade.

### Persistence: what recovery presupposes

Recoverability presupposes the updates reach storage — for a plain feed publisher this
is not a choice but what publishing *is*: each update goes out under a valid postage
stamp, push-synced as usual. The **wrapped node chunk is the essential upload**: it is
what pot descent — recovery, history, seeking — resolves against (the storage-side
counterpart of `swarm-cache-wrapped-chunk`). The SOC upload serves the feed identity:
it makes the stream followable as a plain sequential feed and carries the no-pot
fallback recovery; implementations MAY phase it in later — the wrapped chunk is the
operative upload. Only a live transport (BPS, below) makes persistence optional; a
stream that skips it still gets gap *detection* and dedup for free, but forgoes
recovery and history.

### Carriage over BPS (SWIP-60/61)

Everything above stands on its own. Over BPS the pattern does double duty — each live
message is simultaneously a retrievable feed update — and this is the one place
SWIP-60 enters:

**The carriage rule.** Within a cohort (`FEED_TOPIC` binding, explicit publishers; p2p
frames and WS bridge frames alike) the 32-byte id field carries the 8-byte `IDX`
instead: `TOPIC` is fixed by the channel, so carrying `H(TOPIC ‖ IDX)` would transmit
an opaque digest of a value the receiver needs in the clear — the index is what makes
gaps *visible at the next frame*. Every receiver reconstructs `id`, verifies `sig`,
and where it stores or forwards outside the cohort, reassembles the standard SOC.

**Validation and dedup — no special rules.** A broker (and every verifying receiver)
checks exactly SWIP-60's list: signature recovers an owner in the genesis publisher
list, and the chunk address `a` — computable from channel topic + carried index +
recovered owner — is not a duplicate. **Dedup is on chunk address, as for every
implicit binding, and needs nothing else**: distinct indices give distinct addresses
by construction, so the SWIP-61 dual-parent overlap, make-before-break reparenting and
history replay all dedup through the one existing rule. The broker MUST NOT enforce
index monotonicity or contiguity — frames may arrive reordered, and gap handling is
...
**Recovery on a live stream** is the generic mechanism verbatim — the anchor index is
read off the frame — and it doubles as **withholding evidence**: a parent whose stream
shows gaps its recovered chunks prove existed is caught without a second feed to
compare against.

**API.** SWIP-60's bridge already carries the frame rule: for feed bindings the
inbound and outbound frame prefix is the bare index, signed id
`keccak256(topic ‖ index)`. Amendments:
...
polled. BPS adds push latency at the live edge; it adds nothing structural.

## Rationale

**Why is this not a BPS SWIP?** Because every load-bearing property — one chunk per
update, history from the latest update, seek by key, recovery by descent under one
signature, the anchor check, publish-is-upload — belongs to the feed pattern, not the
transport. BPS contributes exactly two things: push latency (a gap shows at the next
frame instead of the next lookup) and the bare-index carriage rule. A plain feed
publisher and a polling client get everything else unchanged.

**Why carry the index instead of the id (over BPS)?** The id is a hash of the index;
within the channel the topic half of its preimage is known to every party. Carrying
the digest hides the one number that makes sequence position, gaps and recovery
addresses computable — and buys nothing, since the id is reconstructible either way.
The bare index is the same information, usable.
...
for a follower that was offline for the whole outage, not just a blip. And with the
pot profile it is a *cheap* read: the gap-revealing update hands over the index to
...
1. in storage, an update is an ordinary sequential feed update: an unmodified feed
   client publishes and follows the stream with no knowledge of this SWIP;
2. with the pot profile on, publishing update `i` creates exactly one new chunk, whose
...
3. a follower detects any index gap at the next verified update and (persistence on)
   recovers every missed element by pot descent from that update's payload, verifying
   ...
   from in-sequence ones at the application boundary;
4. a follower performs the anchor check — the SOC at its last known index wraps the
...
6. over BPS, a publisher node assembles the SOC from a bare-index frame (reconstruct
   id, sign client-side per SWIP-60's key-holding rule), and every verifying receiver
   validates it with SWIP-60's checks alone — dedup on chunk address, no index
   monotonicity or contiguity enforcement anywhere in the forwarding plane;
...
No new frames, no proto change, no version bump. Storage-side artifacts are ordinary
feeds — legacy feed clients interoperate by construction, which is also why the
pattern needs nothing from BPS. On the BPS side this SWIP realises semantics SWIP-60
reserved for it (id unconstrained under explicit regimes; bare-index frame prefix for
feed bindings). Both BPS SWIPs receive follow-up amendments once this SWIP settles:
SWIP-61's dual-parent requirement (conformance items 5–6) is relaxed to not-required
for self-indexed cohorts — trees mixing single- and dual-parented subscribers were
already well-formed — and SWIP-60's `ANCHOR`-binding dedup note gains the
application-level requirement that payloads carry an index, without which
